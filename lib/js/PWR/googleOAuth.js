  ...
  return new Promise((resolve, reject) => {
   if (app.authenticated) {
    ...
    .then(() => {
     ...
     if (app.authenticated) resolve();
     else reject('Authentication failure: No access to required scope');
    })
    .catch((errorMessage) => reject('Authentication failure: ' + errorMessage));
   }
  });
 }
